  ...
  return <>
    ...
    {panel === "roles" && <div className="didar-role-overlay" onMouseDown={(event) => { if (event.target === event.currentTarget) close() }}>
      <div className="didar-role-dropdown" role="dialog" aria-modal="true" aria-label={copy.myDidar} dir={dir}>
        ...
        <div className="didar-role-dropdown-grid">{(["retailer", "consumer", "supplier", "wholesaler"] as const).map((role, i) => <Link href={`/${locale}/my-didar/${role}`} key={role} onClick={close}><small>0{i + 1}</small><span>{didarUiCopy[locale][role]}</span><b aria-hidden="true">↗</b></Link>)}</div>
      </div>
    </div>}
    {panel === "menu" && <div className="didar-overlay" onMouseDown={(event) => { if (event.target === event.currentTarget) close() }}>
      <aside className="didar-drawer" role="dialog" aria-modal="true" aria-label={copy.menu} dir={dir}>
        ...
        {section === "root" ? <nav className="didar-drawer-links">
          ...
          <Link className="didar-drawer-feature" href={`/${locale}/my-didar`} onClick={close}>{copy.myDidar}<small>{copy.account}</small></Link>
